  ...
  const handleAuth = async (e) => {
    ...
    try {
      if (isLogin) {
        ...
        // --- NOTIFIKASI TOAST KUSTOM DIPINDAH KE SINI ---
        ...
        // REGISTER (Sekarang mengirim Display Name ke Supabase)
        const { error } = await supabase.auth.signUp({
          email,
          password,
          options: {
            data: {
              display_name: displayName, // Menyimpan nama ke database metadata
            },
          },
        });
        if (error) throw error;
  ...
  return (
    <div className="flex justify-center items-center min-h-[75vh]">
      ...
      <div className="bg-[#1a0505]/40 backdrop-blur-xl p-10 rounded-3xl border border-red-900/30 w-full max-w-md shadow-[0_0_40px_rgba(153,27,27,0.15)] relative overflow-hidden">
        {/* Aksen garis merah di atas kartu */}
        ...
        <form onSubmit={handleAuth} className="flex flex-col gap-5">
          {/* Form NAMA PANGGILAN hanya muncul saat REGISTER */}
          {!isLogin && (
            <div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-gradient-to-r from-red-700 to-red-600 hover:from-red-600 hover:to-red-500 text-white font-bold py-3.5 rounded-xl transition-all shadow-[0_0_20px_rgba(220,38,38,0.3)] mt-4 border border-red-500/50"
          >
